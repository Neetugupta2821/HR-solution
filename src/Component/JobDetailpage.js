...
export default function JobDetailpage() {
  ...
  const generatePDF = () => {
    const doc = new jsPDF();
    doc.text(`Job Title: ${information?.job_title}`, 10, 10);
    doc.text(`Job Description: ${information?.job_Description}`, 10, 20);
    doc.text(`Phone: ${information?.phone_no}`, 10, 30);
    doc.text(`Address: ${information?.company_address}`, 10, 40);
    doc.text(`Responsibility: ${information?.job_Responsibility}`, 10, 50);
    doc.text(`Preferred Qualifications: ${information?.key_qualification}`, 10, 60);
    doc.text(`Experience: ${information?.Experience}`, 10, 70);
    doc.text(`Company company name: ${information?.company_name}`, 10, 80);
    doc.text(`Job schedule: ${information?.job_schedule}`, 10, 90);
    doc.text(`Company Industry: ${information?.company_Industry}`, 10, 100);
    doc.save('job-details.pdf');
  };

  // const shareUrl = `${window.location.origin}/jobdetails`;
  const shareUrl = `http://localhost:3000/hrsolution`;
          ...
          <div className="row">
            <div className="col-lg-8">
              {parseInt(information?.template_type) === 2 ? (
                ...
                <div className="pt-50 shortSecJob">
                  <div className="container">
                    <div className="row">
              ...
              <div className="download-button">
                <button onClick={generatePDF}>Download Job Details</button>
              </div>
              <div className="share-buttons">
                <FacebookShareButton url={shareUrl} title={information?.job_title}>
                  <FacebookIcon size={32} round />
                </FacebookShareButton>
                <TwitterShareButton url={shareUrl} title={information?.job_title}>
                  <TwitterIcon size={32} round />
                </TwitterShareButton>
                <LinkedinShareButton url={shareUrl} title={information?.job_title}>
                  <LinkedinIcon size={32} round />
                </LinkedinShareButton>
                <WhatsappShareButton url={shareUrl} title={information?.job_title}>
